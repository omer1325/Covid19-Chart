...
namespace CovidChart.API.Models
{
    public class CovidService
    {
        ...
        public List<CovidChart> GetCovidChartList()
        {
            ...
            {
                command.CommandText = "SELECT tarih, [1],[2],[3],[4],[5] FROM (SELECT[City], [Count], Cast([CovidDate] as date) as tarih " +
                    "FROM Covids) as covidT PIVOT (Sum(Count) For City In([1],[2],[3],[4],[5])) as PTable " +
                    "order by tarih asc";
